...
TMP_VIDEO=$MEDIA_PATH/files.lst

echo Checking data for last day
find -H $MEDIA_PATH -maxdepth 1 -daystart -type f -name "$MEDIA_MASK" -mtime $MTIME -printf "file '%p'\n" | sort >$TMP_VIDEO
...
then
  ...
  $FFMPEG -f concat -safe 0 -i $TMP_VIDEO -c:v $CODEC -b:v $BACKUP_BITRATE -an -y -f mp4 $TARGET_FILE && (
    ...
    find -H $MEDIA_PATH -maxdepth 1 -daystart -type f -name "$MEDIA_MASK" -mtime $MTIME -delete
  )
else
...
find -H $MEDIA_PATH -type f -name "*.mp4" -mtime +$RETAIN_DAYS -delete
echo Finished
